...
# Load API tokens and their usage count from a file
with open('api_tokens.json', 'r') as f:
    api_tokens = json.load(f)

# Iteratively use API tokens
for api_token in api_tokens:
    if api_token['usage'] < api_token['max_usage']:
        # Call the API with this token
        # ...

        # Update the usage count for this token
        api_token['usage'] += 1

        # Write the updated token usage information to the file
        with open('api_tokens.json', 'w') as f:
            json.dump(api_tokens, f)
        break
